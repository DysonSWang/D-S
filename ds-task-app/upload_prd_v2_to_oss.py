#!/usr/bin/env python3
"""
上传 D/s 任务打卡系统 PRD v2.0（合规优化版）到阿里云 OSS
"""
...
oss_object_name = f'ds-task-app/PRD-v2.0-{datetime.now().strftime("%Y%m%d")}.md'
...
if __name__ == '__main__':
    print('=' * 60)
    print('伙伴任务打卡系统 PRD v2.0（合规优化版）上传工具')
    print('=' * 60)
    print()
    ...
    if result:
        ...
        print(f'\n✅ 可以直接将公开访问链接发给专家评审！')
        print(f'\n📝 版本说明：')
        print(f'   - 淡化敏感表述（主人→引导者，服从者→成长者）')
        print(f'   - 增强用户自主性（可主动解除关系、自主领取任务）')
        print(f'   - 新增合规章节（敏感词过滤、年龄验证、内容审核）')
        print(f'   - 定位为「亲密关系成长工具」')
